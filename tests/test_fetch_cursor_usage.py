...
class FetchCursorUsageTests(unittest.TestCase):
    ...
    def test_get_returns_http_error_text_for_non_auth_http_status(self) -> None:
        error = urllib.error.HTTPError(
            url="https://example.com",
            code=500,
        ...
        self.assertIsNone(data)
        self.assertEqual(err, str(error))
    ...
    def test_main_stops_after_tabs_error_and_preserves_prior_inserts(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            db_path = Path(temp_dir) / "cursor_usage.db"
            summary_path = Path(temp_dir) / "cursor_usage_latest.json"
            ...
            try:
                ...
                agent_rows = conn.execute(
                    "SELECT COUNT(*) FROM usage_snapshots WHERE metric_type = 'agent_edits'"
                ).fetchone()[0]
            finally:
                conn.close()

            self.assertEqual(total_rows, 1)
            self.assertEqual(agent_rows, 1)
